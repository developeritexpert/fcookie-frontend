    ...
    ${fixed ? "fixed top-0 left-0 z-40 bg-white dark:bg-black shadow-md" : "relative z-40 bg-transparent shadow-none"}`} >
        <div className="p-[16px] max-w-[1440px] mx-auto bg-[#FFFFFF12] flex justify-between items-center rounded-[11px] border border-[#EFB24D]/10 dark:border-[#444444]">
          ...
          <div className='hidden lg:flex lg:items-center gap-[10px]'>
            ...
            {/* <div>
              ...
              >
                ...
                <div className='bg-[#EFB24D1F] dark:bg-[#FFFFFF12] p-[8px] rounded-[7px]'>
                  {theme === "dark" ? (
          ...
          <div className="lg:hidden flex items-center gap-4">
            <LanguageDropdown />
            <button
              className="hamburger-btn bg-[#EFB24D0F] dark:bg-[#FFFFFF0D] p-3 rounded-[7px]"
              onClick={toggleMobileMenu}
            >
              <div className="w-6 h-6 flex flex-col justify-between">
                <span className={`w-full h-0.5 bg-current rounded transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2.5' : ''
                  }`}></span>
                <span className={`w-full h-0.5 bg-current rounded transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''
                  }`}></span>
                <span className={`w-full h-0.5 bg-current rounded transition-all duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2.5' : ''
                  }`}></span>
              </div>
        ...
        {(isMobileMenuOpen || isAnimating) && (
          <div className="lg:hidden fixed inset-0 z-50">
            ...
            {/* Sidebar */}
            <div
              className={`mobile-menu fixed right-0 top-0 h-full w-80 bg-white dark:bg-[#0D0D0D] shadow-xl transform transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
                }`}
            >
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                ...
                <button
                  onClick={toggleMobileMenu}
                  className="p-2 rounded-lg bg-white dark:bg-[#0D0D0D]  hover:bg-gray-200  transition-colors"
                >
              ...
              <div className="p-6">
                ...
                <div className="space-y-4 border-t border-gray-200 dark:border-gray-700 pt-6">
                  <button
                    onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
                    className="w-full bg-[#EFB24D0F] dark:bg-[#FFFFFF0D]  border border-[#E6E6E6] dark:border-[#343434] cursor-pointer px-4 py-3 text-base flex items-center justify-between font-semibold rounded-[7px]"
                  >
                    <span>{theme === "dark" ? "Dark Mode" : "Light Mode"}</span>
